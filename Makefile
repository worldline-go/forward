...
.PHONY: docs lint fmt test race msan coverage clean help

$(BINARY): docs ## Build the binary file
	go build -trimpath -ldflags="-s -w -X $(PKG)/cmd/forward/main.version=$(VERSION)" -o $(BINARY) cmd/forward/main.go
